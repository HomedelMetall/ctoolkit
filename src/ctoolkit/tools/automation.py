...
class automation:
    ...
    @calculate_time    
    def structs_from_MD(self, MDtrajdict, names, mult):
        num_snaps = len(MDtrajdict['energy'])
        VASPstructs = []
        for i in range(num_snaps):
            sys.stdout.write('%3d/%d\r' % (i,num_snaps))
            sys.stdout.flush()
            struct = VASP()
            # Fill poscar
            B = np.array(MDtrajdict['basis'][i])
            alat = 1.0
            names = names
            mult = mult
            atoms_frac = np.array(MDtrajdict['frac_positions'][i])
            ...
            if(vaspNow.getStatus()['OUTCAR']): 
                ...
                for i, prop in enumerate(props_okay):
                    ...
                    if i < len(props_okay)-1:
                        OUTCAR_tags += ', '
                print('\t%10s loaded! POSCAR %s (V=%.3f). OUTCAR %s (V=%.3f). Read from OUTCAR: %s.' % (dirs, POSCAR_status, vaspNow.POSCAR.volume, OUTCAR_status, vaspNow.outcar.volume, OUTCAR_tags))
            else:
                print('\t%10s loaded! POSCAR %s (V=%.3f). OUTCAR not read.' % (dirs, POSCAR_status, vaspNow.POSCAR.volume))
        return structures
